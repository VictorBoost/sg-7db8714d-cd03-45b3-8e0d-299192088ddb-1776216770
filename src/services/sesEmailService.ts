...
export async function sendEscrowAutoReleaseWindowNotice(
  clientEmail: string,
  clientName: string,
  providerName: string,
  projectTitle: string,
  contractId: string,
  rejectDeadlineIso: string,
  baseUrl: string = "https://bluetika.co.nz"
): Promise<boolean> {
  const deadlineLocal = new Date(rejectDeadlineIso).toLocaleString("en-NZ", {
  ...
  return sendEmail({
    to: clientEmail,
    subject: `Action needed: payout scheduled — ${projectTitle}`,
    htmlBody: baseHtml("Automatic payout notice", `
      <p>Kia ora ${clientName},</p>
      <p>The job <strong>"${projectTitle}"</strong> has completed BlueTika&apos;s evidence and dispute checks with <strong>${providerName}</strong>.</p>
      <div class="highlight">
        <p style="margin:0;"><strong>You have 48 hours</strong> to raise a concern if you do not want this payment to proceed on the next scheduled Friday payout.</p>
      </div>
      <p>If we don&apos;t hear from you by <strong>${deadlineLocal} (NZ)</strong>, your escrow payment will be queued for release under our weekly Friday payout (Wednesday cutoff applies).</p>
      <p>You can still contact BlueTika support after this window if there is an urgent issue—we review manual holds where needed.</p>
      <a href="${baseUrl}/contracts" class="button">Open contract</a>
      <div class="warning">
        <p style="margin:0;font-size:14px;">Prefer to approve sooner? Use &quot;Approve payment&quot; on your contract page.</p>
      </div>
    `, baseUrl),
  });
}
...
export const sesEmailService = {
  ...
  sendEscrowAutoReleaseWindowNotice,
