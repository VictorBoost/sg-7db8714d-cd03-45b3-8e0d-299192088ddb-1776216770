...
export default function AdminDisputes() {
  ...
  const loadDisputes = async () => {
    console.log("🔍 [Disputes] Loading disputes...");
    ...
    try {
      ...
      console.log("🔍 [Disputes] Query result:", {
      ...
      if (error) {
        console.error("❌ [Disputes] Database error:", error);
        throw error;
      }

      const formattedData = data?.map(dispute => ({
        ...dispute,
        contract: Array.isArray(dispute.contract) ? dispute.contract[0] : dispute.contract
      })) || [];

      console.log("✅ [Disputes] Loaded", formattedData.length, "disputes");
