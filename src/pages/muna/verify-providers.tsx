  ...
  const loadDocuments = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          provider:profiles!verification_documents_provider_id_fkey(
            id,
            full_name,
            email,
            phone_number
      ...
      const formattedData = data?.map(doc => ({
        ...doc,
        provider: Array.isArray(doc.provider) ? doc.provider[0] : doc.provider,
        category: Array.isArray(doc.category) ? doc.category[0] : doc.category,
        subcategory: Array.isArray(doc.subcategory) ? doc.subcategory[0] : doc.subcategory
      })) || [];

      console.log("✅ [Verify Providers] Loaded", formattedData.length, "verification documents");
      
      const domesticHelper = formattedData.filter(d => 
        d.category?.name?.toLowerCase().includes("domestic") || 
        d.subcategory?.name?.toLowerCase().includes("domestic")
      );
      const standard = formattedData.filter(d => !domesticHelper.includes(d));
      
      console.log("📊 [Verify Providers] Split:", {
        standard: standard.length,
        domesticHelper: domesticHelper.length
      });

      setStandardDocs(standard);
      setDomesticHelperDocs(domesticHelper);
    } catch (error: any) {
