...
export default function TrustAndSafety() {
  ...
  const loadData = async () => {
    console.log("🔍 [Trust & Safety] Loading data...");
    
    try {
      const [reportsResult, bypassesResult, bannedResult] = await Promise.all([
        supabase
          .from("reports")
          .select(`
            *,
        ...
        supabase
          ...
          .select(`
            *,
            profile:profiles!bypass_attempts_user_id_fkey(full_name, email)
          `)
          .order("created_at", { ascending: false }),
      ...
      console.log("🔍 [Trust & Safety] Query results:", {
        reports: { count: reportsResult.data?.length || 0, error: reportsResult.error?.message },
        bypasses: { count: bypassesResult.data?.length || 0, error: bypassesResult.error?.message },
        banned: { count: bannedResult.data?.length || 0, error: bannedResult.error?.message }
      });

      if (reportsResult.error) throw reportsResult.error;
      if (bypassesResult.error) throw bypassesResult.error;
      ...
      const formattedReports = reportsResult.data?.map(report => {
        ...
        const reported_user = Array.isArray(report.reported_user) ? report.reported_user[0] : report.reported_user;
        return {
          ...report,
          reporter: reporter || { id: report.reporter_id, full_name: 'Unknown', email: '' },
          reported_user: reported_user || null,
          reported_project: null,
          resolved_by_user: null
        };
      }) || [];

      const formattedBypasses = bypassesResult.data?.map(bypass => {
        const profile = Array.isArray(bypass.profile) ? bypass.profile[0] : bypass.profile;
        return {
          id: bypass.id,
          user_id: bypass.user_id,
          user_name: profile?.full_name || 'Unknown',
          user_email: profile?.email || 'Unknown',
          content: bypass.content_attempted,
          detected_patterns: bypass.detected_patterns || [],
          context: bypass.page_location,
          attempt_count: bypass.escalation_level,
          created_at: bypass.created_at
        };
      }) || [];

      console.log("✅ [Trust & Safety] Loaded successfully");
      
      setReports(formattedReports as any);
      setBypassAttempts(formattedBypasses as any);
      
      const formattedSuspended = bannedResult.data?.map(u => ({
        id: u.id,
        user_id: u.id,
        user_name: u.full_name || 'Unknown',
        user_email: u.email || 'Unknown',
        suspension_type: 'permanently_banned',
        reason: 'Banned',
        suspended_at: u.updated_at,
        suspended_until: null,
        is_active: u.is_banned
      })) || [];
      
      setSuspendedAccounts(formattedSuspended as any);
    } catch (error: any) {
      console.error("💥 [Trust & Safety] Load failed:", error);
      toast({
        title: "Error",
