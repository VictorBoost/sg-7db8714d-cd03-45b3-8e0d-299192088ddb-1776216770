...
function getContractFromProject(project: unknown): {
  id?: string;
  status?: string | null;
  payment_status?: string | null;
  reviews?: unknown;
} | null {
  const raw = (project as { contract?: unknown }).contract;
  if (!raw) return null;
...
export default function ProjectDetail() {
  ...
  const loadProject = async () => {
    ...
    if (error) {
      ...
      // Only load bids if user is the project owner
      const session = await authService.getCurrentSession();
      const isOwner = session?.user?.id === data?.client_id;
      
      if (isOwner && data?.bids) {
        ...
        // Not the owner - hide all bids
        setBids([]);
      }
      
      // Check evidence photo status if project has an active contract
      const projectData = data as any;
      const evidenceContract = getContractFromProject(projectData);
      if (evidenceContract?.id && evidenceContract.status === "active") {
        try {
          const status = await getEvidenceStatusSummary(evidenceContract.id);
  ...
  const projectContract = getContractFromProject(project);
  /** Includes `completed` if you add that status; BlueTika escrow typically uses held/released after checkout. */
  const contractPaymentStatus = projectContract?.payment_status;
  const hasPaidContractForContact =
    contractPaymentStatus === "completed" ||
    ...
    <>
      ...
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <ProviderProfileModal
          ...
          hasPaidContract={hasPaidContractForContact}
        />
